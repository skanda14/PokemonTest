...
class Item:
    def __init__(self, name, description, effect_type, effect_value, quantity):
        ...
        self.effect_type = effect_type  # "heal" ou "capture"
        self.effect_value = effect_value  # Taux de capture ou PV soignés
...
def try_capture(target, ball_rate):
    """
    Logique de capture simplifiée :
    Chance = ((MaxHP * 3 - CurrentHP * 2) * BallRate) / (MaxHP * 3)
    """
    chance = ((target.max_hp * 3 - target.hp * 2) * ball_rate) / (target.max_hp * 3)
    # On normalise pour avoir une valeur entre 0 et 1 (ici on divise par 255 pour simuler le jet de dé)
    roll = random.random()
...
class BattleEngine:
            ...
            if item.effect_type == "heal":
                ...
                if try_capture(self.enemy, item.effect_value):
                    ...
                    if len(self.player_party) < 6:
                        self.player_party.append(self.enemy)
                    # On termine le combat
                    self.enemy.hp = 0
                    self.action_queue = []  # Annule l'attaque de l'ennemi
                else:
    ...
    def advance_message(self):
        if self.message_queue:
            ...
            if self.enemy.hp <= 0:
                self.state = "END"
                # Distingue capture de victoire par KO
                if self.enemy in self.player_party:
                    self.current_message = f"{self.enemy.name} a rejoint votre équipe !"
                else:
    ...
    def handle_event(self, event):
        ...
        elif self.state in ["RESOLVING", "END"] or (self.state == "MAIN_MENU" and self.message_queue):
            if event.key == pygame.K_SPACE: self.advance_message()
...
def main():
    ...
    inventory = [
        ...
        Item("Poke Ball", "Capture", "capture", 100, 5)  # 100 est le taux de capture de base
    ]
