...
class BattleEngine:
    def __init__(self, screen, player_party, enemy_pkmn, inventory):
        ...
        # États du jeu: "MAIN_MENU", "MOVE_MENU", "PARTY_MENU", "ITEM_MENU", "RESOLVING", "END"
    ...
    def process_next_action(self):
        ...
        if action["type"] == "switch":
            ...
            item = action["data"]
            item.quantity -= 1
            self.message_queue.append(f"Vous utilisez {item.name} !")

            if item.effect_type == "heal":
                healed_amount = min(item.effect_value, self.player.max_hp - self.player.hp)
                self.player.hp += healed_amount
                self.message_queue.append(f"{self.player.name} récupère {healed_amount} PV !")
                ...
                if event.key == pygame.K_ESCAPE or event.key == pygame.K_BACKSPACE:
                    ...
                    if self.selected_item_index == len(self.inventory):
                        ...
                        if selected_item.quantity <= 0:
                            ...
                            self.advance_message()
                        elif selected_item.effect_type == "heal" and self.player.hp == self.player.max_hp:
                            self.message_queue.append(f"{self.player.name} a déjà tous ses PV !")
                            self.return_state = "ITEM_MENU"
                            self.advance_message()
                        else:
                            self.resolve_turn("item", selected_item)
