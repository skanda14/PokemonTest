...
class ChoiceMenuView:
    ...
    def init_items(self, items=None):
        self.items = items if items is not None else ["OUI", "NON"]
        self.init_items_display()

    def init_items_display(self):
        self.reset()
        self.cursors_display = []
        for i, string in enumerate(self.items):
            x = 2
            y = 1 + i * 2
            self.init_item_display(string,(x, y))
            self.init_cursor_display((x - 1, y))

    def init_item_display(self, name_string, grid_pos):
        new_item_display = []
